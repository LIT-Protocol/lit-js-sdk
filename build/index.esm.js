...
function decryptWithPrivKey(encryptedData, receiverPrivateKey) {
  switch (encryptedData.version) {
    case 'x25519-xsalsa20-poly1305':
      {
        var recieverEncryptionPrivateKey = naclUtil.decodeBase64(receiverPrivateKey); // assemble decryption parameters
  ...
  try {
    ...
    return Promise.resolve(_catch$1(function () {
      return Promise.resolve(connectWeb3()).then(function (_ref9) {
        ...
        var contract = new Contract(tokenAddress, LIT.abi, new Web3Provider(web3).getSigner());
  ...
  try {
    return Promise.resolve(connectWeb3()).then(function (_ref4) {
      ...
      return Promise.resolve(web3.request({
        ...
        var address = verifyMessage(body, signature).toLowerCase();
...
var signAndSaveAuthMessage = function signAndSaveAuthMessage() {
  try {
    ...
    return Promise.resolve(signMessage({
      ...
      var commsKeyPair = nacl.box.keyPair();
...
var connectWeb3 = function connectWeb3() {
  try {
    ...
    return Promise.resolve(detectEthereumProvider()).then(function (provider) {
...
var LitNodeClient = /*#__PURE__*/function () {
  ...
  */
  ...
  _proto.verifyJwt = function verifyJwt(_ref) {
    var jwt = _ref.jwt;
    var pubKey = uint8arrayFromString(this.networkPubKey, 'base16'); // console.log('pubkey is ', pubKey)

    var jwtParts = jwt.split('.');
    var sig = uint8arrayFromString(jwtParts[2], 'base64url'); // console.log('sig is ', uint8arrayToString(sig, 'base16'))

    var unsignedJwt = jwtParts[0] + "." + jwtParts[1]; // console.log('unsignedJwt is ', unsignedJwt)

    var message = uint8arrayFromString(unsignedJwt); // console.log('message is ', message)
  ...
  _proto.getSignedToken = function getSignedToken(_ref2) {
    ...
    try {
      ...
      return Promise.resolve(Promise.all(nodePromises)).then(function (signatureShares) {
        ...
        var pkSetAsBytes = uint8arrayFromString(_this2.networkPubKeySet, 'base16');
        ...
        var finalJwt = unsignedJwt + "." + uint8arrayToString(signature, 'base64url');
        return finalJwt;
      });
    } catch (e) {
      return Promise.reject(e);
  ...
  _proto.saveSigningCondition = function saveSigningCondition(_ref3) {
    ...
    try {
      ...
      return Promise.resolve(hashResourceId(resourceId)).then(function (hashOfResourceId) {
        var hashOfResourceIdStr = uint8arrayToString(new Uint8Array(hashOfResourceId), 'base16'); // hash the access control conditions

        return Promise.resolve(hashAccessControlConditions(accessControlConditions)).then(function (hashOfConditions) {
          var hashOfConditionsStr = uint8arrayToString(new Uint8Array(hashOfConditions), 'base16'); // create access control conditions on lit nodes
    ...
    try {
      ...
      return Promise.resolve(Promise.all(nodePromises)).then(function (decryptionShares) {
        ...
        decryptionShares.forEach(function (s, idx) {
          ...
          var shareAsBytes = uint8arrayFromString(s.decryptionShare, 'base16');
        ...
        var pkSetAsBytes = uint8arrayFromString(_this6.networkPubKeySet, 'base16');
        wasmBlsSdkHelpers.set_mc_bytes(pkSetAsBytes); // set the ciphertext bytes

        var ciphertextAsBytes = uint8arrayFromString(toDecrypt, 'base16');
  ...
  */
  ...
  _proto.saveEncryptionKey = function saveEncryptionKey(_ref5) {
    ...
    try {
      ...
      var encryptedKey = wasmBlsSdkHelpers.encrypt(uint8arrayFromString(_this8.subnetPubKey, 'base16'), symmetricKey);
      ...
      return Promise.resolve(crypto.subtle.digest('SHA-256', encryptedKey)).then(function (hashOfKey) {
        var hashOfKeyStr = uint8arrayToString(new Uint8Array(hashOfKey), 'base16'); // hash the access control conditions
