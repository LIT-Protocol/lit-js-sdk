...
async function importSymmetricKey(symmKey) {
...
async function generateSymmetricKey() {
  const symmKey = await crypto.subtle.generateKey(SYMM_KEY_ALGO_PARAMS, true, ['encrypt', 'decrypt']);
  return symmKey;
}
/**
...
async function decryptWithSymmetricKey(encryptedBlob, symmKey) {
...
async function encryptWithSymmetricKey(symmKey, data) {
...
function decryptWithPrivKey(encryptedData, receiverPrivateKey) {
  switch (encryptedData.version) {
    case 'x25519-xsalsa20-poly1305':
      {
        const recieverEncryptionPrivateKey = naclUtil.decodeBase64(receiverPrivateKey); // assemble decryption parameters
...
async function connectWeb3() {
  ...
  const provider = await detectEthereumProvider(); // trigger metamask popup
...
async function checkAndSignAuthMessage({
  ...
  const chainId = await web3.request({
    method: 'eth_chainId',
    params: []
  });
...
async function findLITs() {
  ...
  try {
    ...
    const chainHexId = await web3.request({
      method: 'eth_chainId',
      params: []
    });
    ...
    const balances = await contract.balanceOfBatch(accounts, tokenIds); // console.log('balances', balances)
...
const listenForChildFrameMessages = async () => {
    ...
    if (childFrameThatSentMessageIndex !== false) {
      ...
      if (event.data.target === 'LitNodeClient') {
        ...
        if (command === 'getEncryptionKey') {
          const encryptionKey = await window.litNodeClient.getEncryptionKey(_extends({}, params));
          window.frames[childFrameThatSentMessageIndex].postMessage({
...
const listenForFrameParentMessages = async () => {
...
async function zipAndEncryptString(string) {
  const zip = new JSZip();
...
async function zipAndEncryptFiles(files) {
  // let's zip em
  const zip = new JSZip();
...
async function decryptZip(encryptedZipBlob, symmKey) {
  ...
  const decryptedZipArrayBuffer = await decryptWithSymmetricKey(encryptedZipBlob, importedSymmKey); // unpack the zip

  const zip = new JSZip();
  const unzipped = await zip.loadAsync(decryptedZipArrayBuffer); // load the files into data urls with the metadata attached
...
async function encryptZip(zip) {
  ...
  const encryptedZipBlob = await encryptWithSymmetricKey(symmKey, zipBlobArrayBuffer); // to download the encrypted zip file for testing, uncomment this
  // saveAs(encryptedZipBlob, 'encrypted.bin')

  const exportedSymmKey = new Uint8Array(await crypto.subtle.exportKey('raw', symmKey));
  console.log('exportedSymmKey in hex', uint8arrayToString(exportedSymmKey, 'base16')); // encrypt the symmetric key with the
...
async function getNpmPackage(packageName) {
  ...
  const blob = await resp.blob(); // console.log('got blob', blob)

  const dataUrl = await fileToDataUrl(blob); // console.log('got dataUrl', dataUrl)
...
async function createHtmlLIT({
  ...
  const formattedAccessControlConditions = accessControlConditions.map(c => canonicalAccessControlConditionFormatter(c)); // console.log('scriptTags: ', scriptTags)
...
async function unlockLitWithKey({
  symmetricKey
}) {
  const mediaGridHolder = document.getElementById('mediaGridHolder');
  const lockedHeader = document.getElementById('lockedHeader'); // convert data url to blob
...
const commitment_sizes_by_threshold = [56, // threshold 0
104, // threshold 1
152, // threshold 2
200, // threshold 3
248, // threshold 4
...
const poly_sizes_by_threshold = [40, // threshold 0
72, // threshold 1
104, // threshold 2
136, // threshold 3
168, // threshold 4
...
const wasmBlsSdkHelpers = new function () {
  ...
  this.encrypt = function (p, m) {
    ...
    try {
      ...
      const ctSize = wasmExports.encrypt(m.length); // get ciphertext bytes
  ...
  this.decrypt = function (s, c) {
    ...
    try {
      ...
      const msgSize = wasmExports.decrypt(c.length); // get message bytes
  ...
  this.combine_signatures = function (mcBytes, sigshares) {
    ...
    for (let shareIndex = 0; shareIndex < sigshares.length; shareIndex++) {
      ...
      const _sigBytes = hexToUint8Array(sigHex);

      const sigIndex = share.shareIndex;

      for (let byteIndex = 0; byteIndex < _sigBytes.length; byteIndex++) {
        const sigByte = _sigBytes[byteIndex]; // NB shareIndex is used instead of sigIndex so we can interate
        // over both
        // SHARE_INDEXES[i]
        // and
  ...
  this.combine_decryption_shares = function (totalShares, mcSize, ctSize) {
    ...
    const msgSize = wasmExports.combine_decryption_shares(totalShares, mcSize, ctSize); // read msg
...
class LitNodeClient {
  ...
  */
  ...
  verifyJwt({
    ...
    const pubKey = uint8arrayFromString(this.networkPubKey, 'base16'); // console.log('pubkey is ', pubKey)

    const jwtParts = jwt.split('.');
    const sig = uint8arrayFromString(jwtParts[2], 'base64url'); // console.log('sig is ', uint8arrayToString(sig, 'base16'))

    const unsignedJwt = `${jwtParts[0]}.${jwtParts[1]}`; // console.log('unsignedJwt is ', unsignedJwt)

    const message = uint8arrayFromString(unsignedJwt); // console.log('message is ', message)
  ...
  */
  ...
  async getSignedToken({
    ...
    signatureShares.sort((a, b) => a.shareIndex - b.shareIndex); // combine the signature shares
    ...
    const unsignedJwt = mostCommonString(signatureShares.map(s => s.unsignedJwt)); // convert the sig to base64 and append to the jwt
  ...
  */
  ...
  async saveSigningCondition({
    ...
    const hashOfResourceIdStr = uint8arrayToString(new Uint8Array(hashOfResourceId), 'base16'); // hash the access control conditions

    const hashOfConditions = await hashAccessControlConditions(accessControlConditions);
    const hashOfConditionsStr = uint8arrayToString(new Uint8Array(hashOfConditions), 'base16'); // create access control conditions on lit nodes
  ...
  */
  ...
  async getEncryptionKey({
    ...
    decryptionShares.sort((a, b) => a.shareIndex - b.shareIndex); // combine the decryption shares
    ...
    const decrypted = wasmBlsSdkHelpers.combine_decryption_shares(decryptionShares.length, pkSetAsBytes.length, ciphertextAsBytes.length); // console.log('decrypted is ', uint8arrayToString(decrypted, 'base16'))
  ...
  */
  ...
  async saveEncryptionKey({
    ...
    console.log('symmetric key encrypted with LIT network key: ', uint8arrayToString(encryptedKey, 'base16')); // hash the encrypted pubkey

    const hashOfKey = await crypto.subtle.digest('SHA-256', encryptedKey);
    const hashOfKeyStr = uint8arrayToString(new Uint8Array(hashOfKey), 'base16'); // hash the access control conditions
